...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($check !== false) {
        // Pindahkan file yang diunggah ke folder pembayaranprivat
        if (move_uploaded_file($_FILES["payment_proof"]["tmp_name"], $target_file)) {
            // Simpan informasi pembayaran ke database
            $sql = "INSERT INTO pembayaranprivat (payment_method, payment_proof) 
                    VALUES ('$payment_method', '$payment_proof')";
            if ($conn->query($sql) === TRUE) {
                // Redirect ke halaman pembayaranprivat setelah pembayaran berhasil
...
<body>
    ...
    <div class="container">
        <div class="payment-form">
            <h1>Halaman Pembayaran Privat</h1>
            <?php if (isset($error_message)): ?>
                <div class="error-message"><?php echo $error_message; ?></div>
            <?php endif; ?>
            <form action="pembayaranprivat.php" method="post" enctype="multipart/form-data">
